...
const SCENARIO_DEFS = {
  soft_landing: {
    ...
    narrative_body: "The MRS's most optimistic credible scenario. Core PCE decelerates toward 2% while GDP holds above 2%. The Fed cuts 75bp over the year, normalizing the yield curve. Credit spreads tighten from their current tight levels. The composite moves from Neutral into Expansion. The inflation pillar — the primary current drag at −0.87z — is the single largest beneficiary as both PCE indicators improve substantially.",
    pillar_detail: {
      Growth: "Payrolls re-accelerate to 175–200K/month. GDP grows 2.2–2.5%. All four growth indicators improve.",
      Inflation: "Core PCE decelerates from ~2.9% to 2.1–2.3%. Both PCE deviation and momentum indicators improve substantially.",
  ...
  inflation_shock: {
    ...
    narrative_body: "Core PCE accelerates from ~2.9% to 4%+ driven by services re-inflation or tariff pass-through. Both inflation indicators (PCE deviation from target and PCE momentum) take severe additional hits — the inflation pillar, already negative, becomes the dominant drag. The Fed responds with 75–100bp of hikes, re-inverting the yield curve. Growth slows with a lag as real income erodes. The composite is likely to enter Slowdown.",
    pillar_detail: {
  ...
  credit_stress: {
    ...
    narrative_body: "Investment-grade spreads widen 150–200bp as risk appetite deteriorates. NFCI rises 0.8 standard deviations. The credit pillar — currently the strongest in the MRS at +0.48z (tight spreads) — takes the primary hit as both spread indicators deteriorate sharply. Liquidity tightens simultaneously. Growth lags 2–3 months as tighter financial conditions pass through to investment and capex.",
    pillar_detail: {
...
function buildHistoricalPath(winId, currentZ, dataThrough, indicatorsData, months = 12) {
  const def = HIST_WINDOWS[winId];
...
function getScenarioPath(scenId) {
  ...
  if (HIST_WINDOWS[scenId])         return buildHistoricalPath(scenId, _currentZ, _dataThrough, _indicatorsData);
...
function renderNarrative(scenPath) {
  const isCustom = activeScenarioId === "custom";
  const isHist   = !!HIST_WINDOWS[activeScenarioId];
  const def      = isCustom ? null : isHist ? HIST_WINDOWS[activeScenarioId] : SCENARIO_DEFS[activeScenarioId];
  ...
  // Find peak: for historical, worst composite; for macro, specified peak_month
  let peakIdx;
  if (isHist) {
    peakIdx = scenPath.reduce((best, row, i) =>
      row.composite < scenPath[best].composite ? i : best, 0);
  } else {
    const peakT = def?.peak_month || 4;
    peakIdx = Math.min(peakT, scenPath.length) - 1;
  }
  ...
  const pillarRows = def?.pillar_detail
    ? Object.entries(def.pillar_detail).map(([k, v]) =>
        `<div class="sv2-pillar-line">
           ...
           <span class="sv2-pillar-text">${v}</span>
         </div>`).join("")
    : "";
...
function renderPillarChart(scenPath) {
  const isCustom = activeScenarioId === "custom";
  const isHist   = !!HIST_WINDOWS[activeScenarioId];
  const color    = isCustom ? "#546e7a"
                 : isHist   ? HIST_WINDOWS[activeScenarioId]?.color
                 : SCENARIO_DEFS[activeScenarioId]?.color || "#546e7a";
  ...
  if (isHist) {
    ...
    const peakT = isCustom ? CUSTOM_STATE.peak_month : (SCENARIO_DEFS[activeScenarioId]?.peak_month || 4);
    peakIdx = Math.min(peakT, scenPath.length) - 1;
  }

  // All pillar scores are now computed correctly via indicator → pillar chain
  const todayPillars     = computePillarScores(_currentZ);
  const baseline6Pillars = _baselinePath[5]?.pillars || todayPillars;
  const scenPeakPillars  = scenPath[peakIdx]?.pillars || baseline6Pillars;

  scenarioPillarChart("sv2-pillar-chart", todayPillars, baseline6Pillars, scenPeakPillars, color);
}
...
function renderIndicatorTable(scenPath) {
  const isCustom = activeScenarioId === "custom";
  const isHist   = !!HIST_WINDOWS[activeScenarioId];
  const def      = !isCustom && !isHist ? SCENARIO_DEFS[activeScenarioId] : null;
  ...
  if (captEl) captEl.textContent = isHist
    ? `Scenario = today's z-scores + actual indicator deltas from ${HIST_WINDOWS[activeScenarioId].label} episode at ${peakLabel}. Z-scores flow through pillar → composite scoring as normal.`
...
async function main() {
  ...
  // Compute today's composite via the scoring engine (not from metadata)
  const todayPillars = computePillarScores(_currentZ);
